...
# ★ v10.8: 5단 쿨다운
# ★ V10.16 FIX: T3~T5 단축 — 깊은 DCA일수록 빨리 평단 압축 필요
# 기존 T4=45분 T5=60분 → HARD_SL 도달 전 DCA 기회 상실 방지
DCA_COOLDOWN_BY_TIER = {2: 1800, 3: 1200, 4: 600, 5: 300}
...
ENABLE_TP1_PRE  = False  # ★ v10.19: 분할매도 비활성화 (슬롯 낭비 방지)

# ★ v10.14c: min_roi 반등 α — worst_roi + α ≥ current_roi → TP1
# T1/T2: 승률 높으므로 넉넉히 (기존 고정TP와 유사)
# T3~T5: DCA 깊은 구간, 빠른 탈출 (바닥 대비 1.5% 반등)
REBOUND_ALPHA = {1: 10.0, 2: 10.0, 3: 7.5, 4: 7.5, 5: 7.5}  # ★ 최종 아키텍처: T1/2=10.0 T3~=7.5
