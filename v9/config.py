...
VERSION = "13.1"  # ★ V13.1 [05-06]: MR T1 hard SL -2.5%, T1 사다리는 HEDGE_COMP만 적용 (사용자 결정)
...
T2_DEFENSE_LADDER = [
    # ★ V13.1 [05-06]: HEDGE_COMP T1 전용 사다리 (MR T1은 HARD_SL_BY_TIER[1]=-2.5%만 사용)
    #   사용자 결정: "T1 사다리는 트랜드 컴프만 하고 mr 은 t1 하드 sl 2.5"
    #   사다리 사양은 V11 hf3 그대로 (사용자 통찰: T1 worst -0.5~-1.0 영역 89% 회복 [실측])
    #   plan_t2_defense_v2 분기에서 role==CORE_MR_HEDGE만 처리하도록 차단
    (-1.0, -0.5, "SL"),
    (-1.2, -0.9, "SL"),
    (-1.4, None, "HARD_SL"),
]
...
HARD_SL_BY_TIER = {1: -2.5, 2: -3.0, 3: -5.5}  # ★ V13.1 [05-06]: T1=-2.5 (사용자 변경: T1 사다리 HEDGE_COMP 전용으로 분리, MR은 T2 DCA 트리거(-1.8) 통과 위해 hard SL 멀리)
