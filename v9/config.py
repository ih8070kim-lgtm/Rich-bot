...
# ═══════════════════════════════════════════════════════════════════
DCA_WEIGHTS = [33, 67]  # ★ V10.31AO: T3 제거
...
DCA_ENTRY_ROI_BY_TIER = {2: -1.0}  # ★ V10.31AO: T2만, T3 제거
...
#     -3.0%          HARD_SL (즉시 전량 컷, 무한 보유 차단)
#
#   변경 의도 (사용자):
#     - T3 제거로 단순화 (T1+T2만 운영)
#     - T2 단계에서 사다리로 단계적 보호
#     - 마지막 -3.0 HARD_SL은 안전망 (회복 못 한 케이스 무한 보유 차단)
#
...
TRIM_BLENDED_ROI_BY_TIER = {2: 1.0}  # ★ V10.31AO: T3 제거 (스켈핑). T2 +1.0% 그대로 (사용자 결정 — 데이터 검증 +1.5/+1.0 유지). 디펜스 모드(T2_DEFENSE_LADDER)는 별도.
...
HARD_SL_BY_TIER = {1: -3.8, 2: -3.0}  # ★ V10.31AO: T3 제거 (스켈핑). HSL_T2 -3.0%는 디펜스 사다리(-3.0 HARD_SL)와 일관.
...
OPEN_CORR_MIN_30M       = 0.50   # ★ V10.31AO-hf4 [05-01]: 0.60 → 0.50 환원 (사용자 결정 — 거래 빈도 우선, SHADOW 데이터로 사후 분석)
