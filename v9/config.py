...
T2_DEFENSE_LADDER = [
    ...
    # ★ V11 hf3 [05-05]: 사용자 사양 (-1.0/-0.5, -1.2/-0.9, -1.4 HARD_SL)
    #   사용자 통찰 [실측 검증]:
    #     T1 worst -0.5~-1.0% 영역: 9건 중 8건이 결국 TP 익절 (89%)
    #     → 이 영역 건드리면 익절 박탈
    #     → 사다리는 -1.0% 이하부터만 발동
    #
    #   동작 (T1에 적용, 명칭은 호환용):
    #     worst ≤ -1.0% + max ≥ -0.5% 회복 시 → -0.5% limit cut
    #     worst ≤ -1.2% + max ≥ -0.9% 회복 시 → -0.9% limit cut
    #     worst ≤ -1.4% → HARD_SL 즉시 market cut
    (-1.0, -0.5, "SL"),
    (-1.2, -0.9, "SL"),
    (-1.4, None, "HARD_SL"),
]
...
HARD_SL_BY_TIER = {1: -1.4}  # ★ V11 hf3 [05-05]: 사다리 -1.4% HARD_SL과 일관
