...
VERSION = "10.31AO"  # ★ V10.31AO: 스켈핑 패러다임 — T3 제거, T1=33/T2=67, T2 디펜스 사다리(-1.5/-2.0/-2.5/-3.0), DCA -1.0%
...
HEDGE_COMP_ENABLED   = False  # ★ V10.31AD: HEDGE 재비활성 — 사용자 판단, V10.31AC 롤백. 실전 관찰 결과 MR FC 손실이 헷지 이익보다 크게 우세. 신규 진입만 차단, 이미 열린 포지션은 기존 TP1/trim/trail 경로로 자연 청산
...
#   2. SL: -0.7% (단일 임계, 사다리 X) ★ 시뮬 최고
#   3. TP: +1.5% (TP1 단일, TRIM X)
#   4. PTP: 비활성 (V10.31AO-hf15 유지)
# 
# [실측] 5일 105건 시뮬 근거:
#   현재 봇 (DCA + 사다리): -$31 / 5일
#   V11 (-0.7/+1.5): +$495 / 5일, WR 50%, 손익비 1:2.1
#   V11 (-1.0/+1.5): +$344 / 5일, WR 51%, 손익비 1:1.5
#   V11 (-0.7/+1.8): +$502 / 5일 (-0.7/+1.5와 차이 미미)
#   
# 손익분기 WR (1:2.1) = 32%
# 실측 50% > 32% = 18%p 여유, 안정 흑자 영역
# 
# 위험 요소:
#   - SL -0.7% 좁음 → slippage [추정 -$30~$50/월]
#   - 일시적 spike에 잘못 cut [추정 5~10% 빈도]
#   - 5일 over-fitting 위험 (한 달 평균 다를 수)
# ═══════════════════════════════════════════════════════════════════
DCA_WEIGHTS = [100]  # ★ V11: T1만 100% (DCA 제거)
...
DCA_ENTRY_ROI_BY_TIER = {}  # ★ V11: DCA 자체 제거
...
T2_DEFENSE_LADDER = []  # ★ V11: 사다리 제거 (T1 단일 진입, SL -1% 단일 임계)

# ★ V10.31AO: T3_DEFENSE_LADDER deprecated (T3 제거)
#   호환성 위해 빈 리스트 유지 — calc_t3_defense_action는 None 반환
T3_DEFENSE_LADDER = []  # ★ V10.31AO: 비활성
...
HARD_SL_BY_TIER = {1: -0.7}  # ★ V11: T1 단일 진입, SL -0.7% (손익비 1:2.1, 시뮬 최고)
